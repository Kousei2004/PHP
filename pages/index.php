...
<!-- Tab Bar -->
<div class="tab-bar">
    <div class="tab active" data-tab="newest">🆕 MỚI NHẤT</div>
    <div class="tab" data-tab="movie">🎬 PHIM RẠP</div>
    <div class="tab" data-tab="series">📺 PHIM BỘ</div>
</div>
...
<div class="movie-grid-container">
    <div class="movie-grid" id="movie-grid">
            ...
            $totalQuery = mysqli_query($conn, "SELECT COUNT(*) as total FROM movies");
            $totalRow = mysqli_fetch_assoc($totalQuery);
            $totalMovies = $totalRow['total'];
            $totalPages = ceil($totalMovies / $moviesPerPage);
            
            // Get movies for current page
            $result = mysqli_query($conn, "SELECT * FROM movies ORDER BY id DESC LIMIT $offset, $moviesPerPage");
        ...
        <div class="pagination">
            ...
            <?php if ($page < $totalPages): ?>
                <a href="?page=<?php echo $page + 1; ?>">Tiếp</a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
</div>
...
<script>
    ...
    tab.addEventListener('click', function() {
        ...
        // Lấy loại tab (newest, movie, series, all)
        const tabType = this.getAttribute('data-tab');
        
        // Lọc phim theo tab
        filterMoviesByTab(tabType);
    });
});
    ...
    // Lọc phim theo loại tab
    let filteredMovies = allMovies.filter(movie => {
        let movieType = movie.getAttribute("data-type") || "movie";
        
        switch(tabType) {
            case 'newest':
                return true; // Hiển thị tất cả phim mới
            case 'movie':
    ...
    // Hiển thị 20 phim đầu tiên sau khi lọc
